...
- Every team action lands in a timestamped, filterable log.
- Main agent reads it to catch up rather than calling a status meeting. <!-- main agent doesn't read it to catch up. this is a much more powerful, and somewhat more low-level tool in the main agent's disposal. besides, `teamstatus` is a go to for catching up. The team log is unique in that it's the only tool that shows what HAS happened as opposed to what is happening now (it's a time series), AND the information the main agent consumes was not generated by any teammate (by way of a teammate setting its own status, or responding to the main agent's question). it's bare-bones data (flat unified tool calling history of the team). And it's exhaustive. So it's a debugging tool, not for catching up. It's not social. Typically it solves the failure state where something isn't working for the team, and root cause can be found only with access to a birdseye view of what has took place. -->
